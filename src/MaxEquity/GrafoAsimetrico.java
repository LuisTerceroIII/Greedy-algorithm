package MaxEquity;

import java.util.HashSet;
import java.util.Set;

public class GrafoAsimetrico {
	
	int[][] A;

	public GrafoAsimetrico(int rows, int columns)  {
		
		verficarVerticesConstructor(rows,columns);
		A = new int [rows][columns];
		
	}
	
	public void agregarArista(int i, int j, int peso) {		
		verficarVertices(i , j);
		//verificarIguales(i, j);
		verificarPeso(peso);
		
		A[i][j] = peso;
		//A[j][i] = peso;		
	}
	
	public void quitarArista(int i, int j) {
		verficarVertices(i , j);
		verificarIguales(i, j);
		
		A[i][j] = 0;
		//A[j][i] = 0;	
		
	}
	
	public boolean existeArista(int i, int j) {
		verficarVertices(i , j);
		//verificarIguales(i, j);
		
		return A[i][j] != 0;
	}
	
	public void setPeso(int i, int j, int newPeso) {
		A[i][j] = newPeso;
	}
	public int getPeso(int i, int j) {
		return A[i][j];
	}
	
	public int vertices() {
		System.out.println("LENGGGTH " + A[0].length);
		return A[0].length;
	}
	
	public Set<Integer> vecinos(int i){
		
		//verficarVertices(i, i);		
		Set<Integer> ret = new HashSet<>();
		
		for(int j = 0; j<vertices(); j++){
			if(existeArista(i, j)) {
				ret.add(j);
			}
		}
		return ret;		
	}
	
	public void print() {
		for (int i = 0; i < A.length; i++) {
		    for (int j = 0; j < A[i].length; j++) {
		        System.out.print(A[i][j] + " ");
		    }
		    System.out.println();
		}
	}

	private void verificarPeso(int peso) {
		if (peso < 1) {
			throw new IllegalArgumentException("El peso no puede ser menor que 1, peso ingresado: " + peso);
		}		
	}

	private void verificarIguales(int i, int j) {
		if (i == j) {
			throw new IllegalArgumentException("Los vertices no pueden ser iguales. ");
		}		
	}

	private void verficarVertices(int i, int j) {
		if (i < 0 || i > A.length) {
			throw new IllegalArgumentException("Valor invalido para: " + i); 
		}
		if (j < 0 || j > A.length) {
			throw new IllegalArgumentException("Valor invalido para: " + j); 
		}		
	}

	private void verficarVerticesConstructor(int rows, int columns ) {
		if (rows <= 0 || columns <= 0) {
			throw new IllegalArgumentException("La cantidad de vertices es incorrecta: " + rows);
		}
		
	}

}
